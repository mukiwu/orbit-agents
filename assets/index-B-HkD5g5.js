...
class Translator extends EventEmitter {
  ...
  translate(keys2, o, lastKey) {
    ...
    keys2 = keys2.map((k2) => typeof k2 === "function" ? keysFromSelector(k2, {
      ...this.options,
      ...opt
    }) : String(k2));
  ...
  resolve(keys2, opt = {}) {
    ...
    if (Array.isArray(keys2)) keys2 = keys2.map((k2) => typeof k2 === "function" ? keysFromSelector(k2, {
      ...this.options,
      ...opt
    }) : k2);
    keys2.forEach((k2) => {
  ...
  getResource(code2, ns, key, options = {}) {
    if (this.i18nFormat?.getResource) return this.i18nFormat.getResource(code2, ns, key, options);
...
class LanguageUtil {
  ...
  getScriptPartFromCode(code2) {
    ...
    if (!code2 || !code2.includes("-")) return null;
  ...
  getLanguagePartFromCode(code2) {
    code2 = getCleanedCode(code2);
    if (!code2 || !code2.includes("-")) return code2;
    const p2 = code2.split("-");
    return this.formatLanguageCode(p2[0]);
  }
  formatLanguageCode(code2) {
    if (isString$1(code2) && code2.includes("-")) {
      let formattedCode;
      try {
  ...
  isSupportedCode(code2) {
    ...
    return !this.supportedLngs || !this.supportedLngs.length || this.supportedLngs.includes(code2);
  }
  ...
  toResolveHierarchy(code2, fallbackCode) {
    ...
    if (isString$1(code2) && (code2.includes("-") || code2.includes("_"))) {
    ...
    fallbackCodes.forEach((fc2) => {
      if (!codes.includes(fc2)) addCode(this.formatLanguageCode(fc2));
    });
    return codes;
  }
}
...
class Formatter {
  ...
  format(value, format, lng, options = {}) {
    ...
    for (let i = 0; i < rawFormats.length; i++) {
      let f2 = rawFormats[i];
      while (f2.indexOf("(") > -1 && !f2.includes(")") && i + 1 < rawFormats.length) {
        f2 = `${f2}${this.formatSeparator}${rawFormats[++i]}`;
      }
      formats.push(f2);
    }
...
function simpleToCron(frequency, intervalValue, intervalUnit, time, weekdays, _weekInterval, monthDay) {
...
function getScheduleDescription(frequency, intervalValue, intervalUnit, time, weekdays, weekInterval, monthDay, t2) {
  switch (frequency) {
    case "interval": {
      const unit = t2(intervalUnit === "minutes" ? "schedule.unit.minutes" : "schedule.unit.hours");
      return t2("schedule.desc.everyN", { value: intervalValue, unit });
    }
    case "daily":
      return t2("schedule.desc.dailyAt", { time });
    case "weekly": {
      const dayNames = weekdays.map((d) => WEEKDAYS.find((w2) => w2.value === d)).filter((w2) => w2 !== void 0).map((w2) => t2(w2.labelKey)).join(t2("schedule.desc.daySeparator"));
      if (weekInterval === 1) {
        return t2("schedule.desc.weekly", { days: dayNames, time });
      }
      return t2("schedule.desc.weeklyEveryN", { n: weekInterval, days: dayNames, time });
    }
    case "monthly":
      return t2("schedule.desc.monthly", { day: monthDay, time });
    default:
      return "";
  }
}
...
function useTasks() {
  ...
  const fetchTasks = reactExports.useCallback(async () => {
    try {
      setLoading(true);
  ...
  const createTask = reactExports.useCallback(async (input) => {
    const task = await api.invoke("task:create", input);
    setTasks((prev) => [task, ...prev]);
    return task;
  }, []);
  const updateTask = reactExports.useCallback(async (input) => {
    const task = await api.invoke("task:update", input);
    setTasks((prev) => prev.map((t2) => t2.id === task.id ? task : t2));
    return task;
  }, []);
  const deleteTask = reactExports.useCallback(async (id2) => {
    await api.invoke("task:delete", id2);
    setTasks((prev) => prev.filter((t2) => t2.id !== id2));
  }, []);
  const toggleTask = reactExports.useCallback(async (id2) => {
    const task = await api.invoke("task:toggle", id2);
    setTasks((prev) => prev.map((t2) => t2.id === task.id ? task : t2));
    return task;
  }, []);
  const runTaskNow = reactExports.useCallback(async (id2) => {
    return api.invoke("task:run-now", id2);
  }, []);
  return {
    tasks,
    loading,
...
function useExecutionLogs(taskId, limit = 100) {
  ...
  reactExports.useEffect(() => {
    const handleUpdate = (log) => {
      setLogs((prev) => {
        const exists = prev.find((l2) => l2.id === log.id);
        if (exists) {
          return prev.map((l2) => l2.id === log.id ? { ...l2, ...log } : l2);
        }
        return [log, ...prev];
      });
    };
    api.on("execution:update", handleUpdate);
    return () => {
      api.off("execution:update", handleUpdate);
    };
  }, []);
  const deleteLogs = reactExports.useCallback(async (ids) => {
    try {
      await api.invoke("log:delete", ids);
      setLogs((prev) => prev.filter((l2) => !ids.includes(l2.id)));
    } catch (err) {
...
function useExecutionLog(id2) {
  ...
  const fetchLog = reactExports.useCallback(async () => {
    try {
      setLoading(true);
      const data = await api.invoke("log:get", id2);
      setLog(data);
      setError(null);
    } catch (err) {
...
function useSettings() {
  ...
  const updateSettings = reactExports.useCallback(async (newSettings) => {
    await api.invoke("settings:update", newSettings);
    setSettings((prev) => ({ ...prev, ...newSettings }));
  }, []);
  const testEmail = reactExports.useCallback(async (toAddress) => {
    await api.invoke(
      "settings:test-email",
      toAddress
    );
  }, []);
  return { settings: settings2, loading, error, fetchSettings, updateSettings, testEmail };
}
...
function TaskForm({ task, onClose, onSaved, variant = "modal" }) {
  ...
  const [loadingMcps, setLoadingMcps] = reactExports.useState(false);
  const initialSchedule = reactExports.useMemo(() => {
  ...
  const [scheduleMode, setScheduleMode] = reactExports.useState(initialSchedule.mode);
  const [frequency, setFrequency] = reactExports.useState(initialSchedule.frequency);
  const [intervalValue, setIntervalValue] = reactExports.useState(initialSchedule.intervalValue);
  const [intervalUnit, setIntervalUnit] = reactExports.useState(initialSchedule.intervalUnit);
  const [scheduleTime, setScheduleTime] = reactExports.useState(initialSchedule.time);
  const [selectedWeekdays, setSelectedWeekdays] = reactExports.useState(initialSchedule.weekdays);
  const [weekInterval, setWeekInterval] = reactExports.useState(task?.week_interval || initialSchedule.weekInterval);
  const [monthDay, setMonthDay] = reactExports.useState(initialSchedule.monthDay);
  ...
  const scheduleDescription = reactExports.useMemo(() => {
    return getScheduleDescription(frequency, intervalValue, intervalUnit, scheduleTime, selectedWeekdays, weekInterval, monthDay, t2);
  }, [frequency, intervalValue, intervalUnit, scheduleTime, selectedWeekdays, weekInterval, monthDay, t2]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const input = {
        ...
        mcp_tools: formData.mcp_tools.length > 0 ? formData.mcp_tools : void 0,
      ...
      setError(err instanceof Error ? err.message : t2("taskForm.errorSave"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const content2 = /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `${variant === "modal" ? "bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden" : "h-full flex flex-col"}`, onClick: (e) => e.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `px-6 py-4 border-b border-gray-100 flex items-center justify-between ${variant === "panel" ? "" : ""}`, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-base font-semibold text-gray-900", children: task ? t2("taskForm.editTitle") : t2("taskForm.newTitle") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500 mt-0.5", children: t2("taskForm.subtitle") })
      ] }),
      variant === "modal" && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          onClick: onClose,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("form", { id: "task-form", onSubmit: handleSubmit, className: `overflow-y-auto ${variant === "modal" ? "max-h-[calc(90vh-140px)]" : "flex-1 min-h-0"}`, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-6 space-y-5", children: [
      error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-red-50 border border-red-200 rounded-lg p-3 text-red-700 text-sm flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-4 h-4 flex-shrink-0", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
        error
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-600 mb-1.5", children: [
          t2("taskForm.name.label"),
          " ",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            placeholder: t2("taskForm.name.placeholder")
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-600 mb-1.5", children: [
          t2("taskForm.description.label"),
          " ",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            className: "w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors",
            placeholder: t2("taskForm.description.placeholder")
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-600", children: [
            t2("taskForm.schedule.label"),
            " ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-red-500", children: "*" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex bg-gray-100 rounded-md p-0.5", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
                onClick: () => setScheduleMode("simple"),
                className: `px-2 py-1 text-sm font-medium rounded transition-all ${scheduleMode === "simple" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`,
                children: t2("taskForm.schedule.simple")
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                className: `px-2 py-1 text-sm font-medium rounded transition-all ${scheduleMode === "advanced" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`,
                children: t2("taskForm.schedule.advanced")
              }
            )
          ] })
        ] }),
        scheduleMode === "simple" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3 bg-gray-50/70 rounded-lg p-3 border border-gray-200/60", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-1.5", children: [
            { value: "interval", labelKey: "freqInterval", Icon: RefreshCw },
            { value: "daily", labelKey: "freqDaily", Icon: Sun },
            { value: "weekly", labelKey: "freqWeekly", Icon: Calendar },
            { value: "monthly", labelKey: "freqMonthly", Icon: CalendarDays }
          ].map((f2) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              type: "button",
              onClick: () => setFrequency(f2.value),
              className: `flex-1 flex items-center justify-center gap-1.5 px-2 py-1.5 text-sm font-medium rounded-md border transition-all ${frequency === f2.value ? "bg-blue-100 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`,
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(f2.Icon, { className: "w-3.5 h-3.5" }),
                t2(`taskForm.schedule.${f2.labelKey}`)
              ]
            },
            f2.value
          )) }),
          frequency === "interval" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600", children: t2("taskForm.schedule.every") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "number",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex bg-white border border-gray-200 rounded-md overflow-hidden", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
                  onClick: () => setIntervalUnit("minutes"),
                  className: `px-2 py-1.5 text-sm font-medium transition-colors ${intervalUnit === "minutes" ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-50"}`,
                  children: t2("taskForm.schedule.minutes")
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  className: `px-2 py-1.5 text-sm font-medium transition-colors ${intervalUnit === "hours" ? "bg-blue-100 text-blue-700" : "text-gray-600 hover:bg-gray-50"}`,
                  children: t2("taskForm.schedule.hours")
                }
              )
            ] })
          ] }),
          frequency === "daily" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600", children: t2("taskForm.schedule.at") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "time",
          ...
          frequency === "weekly" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600", children: weekInterval === 1 ? t2("taskForm.schedule.week") : t2("taskForm.schedule.weeks") })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600 block mb-1.5", children: t2("taskForm.schedule.on") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-1", children: WEEKDAYS.map((day) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  type: "button",
                  onClick: () => toggleWeekday(day.value),
                  className: `flex-1 py-1.5 text-sm font-medium rounded-md border transition-all ${selectedWeekdays.includes(day.value) ? "bg-blue-100 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500 hover:bg-gray-50"}`,
                  title: t2(day.fullLabelKey),
                  children: t2(day.labelKey)
                },
                day.value
              )) })
            ] })
          ] }),
          frequency === "monthly" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600", children: t2("taskForm.schedule.at") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-gray-600", children: t2("taskForm.schedule.onDay") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "select",
                {
                  value: monthDay,
                  ...
                  className: "px-2 py-1.5 text-sm bg-white border border-gray-200 rounded-md focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500",
                  children: Array.from({ length: 31 }, (_, i) => i + 1).map((d) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: d, children: d }, d))
                }
              )
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 pt-1 border-t border-gray-200/60", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-3.5 h-3.5 text-blue-500", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm text-blue-600 font-medium", children: scheduleDescription })
          ] })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-2 text-sm text-gray-400", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-3.5 h-3.5 mt-0.5 flex-shrink-0", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium text-gray-500", children: t2("taskForm.schedule.cronFormat") }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "mt-1", children: [
                t2("taskForm.schedule.examplesLabel"),
                " ",
                /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-gray-100 px-1 rounded", children: "0 9 * * *" }),
                " ",
                t2("taskForm.schedule.exampleDailyDesc"),
                ", ",
                /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-gray-100 px-1 rounded", children: "*/15 * * * *" }),
                " ",
                t2("taskForm.schedule.exampleIntervalDesc")
              ] })
            ] })
          ] })
        ] })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                ...
                projectPath ? t2("taskForm.skills.changeProject") : t2("taskForm.skills.selectProject")
              ]
            }
          )
        ] }),
        projectPath && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mb-2 px-2.5 py-1.5 bg-blue-50 border border-blue-200 rounded-lg text-sm", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(FolderOpen, { className: "w-3.5 h-3.5 text-blue-500 flex-shrink-0" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-blue-700 truncate flex-1", title: projectPath, children: projectPath }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2 flex-wrap", children: [
            ...
            "button",
            {
              ...
              className: `flex-1 flex items-center justify-center p-2.5 h-14 border rounded-lg transition-all ${formData.cli_tool === tool.value ? "bg-blue-50 border-blue-300 text-blue-700 shadow-sm" : "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100"}`,
              children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-medium text-sm", children: tool.label })
            },
            tool.value
          )) })
        ] }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              type: "button",
              onClick: async () => {
                const files = await window.electronApi.invoke("dialog:open-files");
                if (files.length > 0) {
                  setFormData((prev) => ({
                    ...prev,
              ...
              children: [
                ...
                t2("taskForm.attachments.addFiles")
              ]
            }
          ),
          formData.attachments.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-wrap gap-1.5", children: formData.attachments.map((filePath, index2) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5 bg-gray-100 px-2 py-1 rounded-md text-sm text-gray-700", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "truncate max-w-[150px]", title: filePath, children: filePath.split("/").pop() }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                type: "button",
                onClick: () => {
                  setFormData((prev) => ({
                    ...prev,
                    attachments: prev.attachments.filter((_, i) => i !== index2)
                  }));
                },
                className: "text-gray-400 hover:text-red-500 transition-colors",
                children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-3 h-3", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M6 18L18 6M6 6l12 12" }) })
              }
            )
          ] }, index2)) })
        ] })
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-600 mb-2", children: t2("taskForm.output.label") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: ["log", "both"].map((type) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            type: "button",
            onClick: () => setFormData((prev) => ({ ...prev, output_type: type })),
            className: `flex-1 flex items-center justify-center gap-1.5 px-3 py-2 border rounded-lg transition-all text-sm font-medium ${formData.output_type === type ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100"}`,
            children: [
              type === "log" && t2("taskForm.output.logOnly"),
              type === "both" && t2("taskForm.output.logAndEmail")
            ]
          },
          type
        )) })
      ] }),
      formData.output_type === "both" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-600 mb-1.5", children: [
          t2("taskForm.emailTo.label"),
          " ",
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-red-500", children: "*" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
            ...
            className: "w-full px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors",
            placeholder: t2("taskForm.emailTo.placeholder")
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        ...
        formData.knowledge_file && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("taskForm.knowledge.description") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                placeholder: t2("taskForm.knowledge.placeholder")
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: t2("taskForm.knowledge.browse")
              }
            )
          ] })
        ] })
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 pt-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            className: `relative w-9 h-5 rounded-full transition-colors ${formData.enabled ? "bg-blue-600" : "bg-gray-300"}`,
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              "span",
              {
                className: `absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${formData.enabled ? "translate-x-4" : "translate-x-0"}`
              }
            )
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm text-gray-600", children: t2("taskForm.enable.label") })
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-6 py-4 border-t border-gray-100 flex justify-end gap-2 bg-white flex-shrink-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          type: "button",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          ...
          children: [
            ...
            task ? t2("taskForm.saveChanges") : t2("taskForm.createTask")
          ]
        }
      )
    ] })
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4", onClick: onClose, children: content2 });
}
function TaskList({
  onEditTask,
  onNewTask,
  ...
  const [runningTasks, setRunningTasks] = reactExports.useState(/* @__PURE__ */ new Set());
  ...
  const handleRunNow = async (e, taskId) => {
    e.stopPropagation();
  ...
  const handleDelete2 = async (e, taskId) => {
    e.stopPropagation();
    const found = tasks.find((task) => task.id === taskId);
    const name2 = found?.name ?? taskId;
    if (!confirm(t2("taskList.confirmDelete", { name: name2 }))) {
      return;
    }
  ...
  if (loading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-center h-full", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-3 text-sm text-gray-500", children: t2("taskList.loading") })
    ] });
  }
  if (error) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4 text-red-700 m-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "font-medium", children: t2("taskList.error") }),
      " ",
      error
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-full gap-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-80 flex-shrink-0 flex flex-col gap-1", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mb-4 px-2 flex items-center justify-between", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-lg font-bold text-gray-900", children: t2("taskList.title") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-400 mt-1", children: t2("taskList.subtitle") })
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto space-y-2 pr-2", children: tasks.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center py-12 px-4 border-2 border-dashed border-gray-200 rounded-xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("taskList.emptyState") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onNewTask, className: "mt-2 text-blue-600 text-sm font-medium hover:underline", children: t2("taskList.createOne") })
      ] }) : tasks.map((task) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            className: `group w-full text-left p-3 rounded-xl transition-all border cursor-pointer relative ${isSelected ? "bg-white shadow-md border-blue-200 ring-1 ring-blue-100 z-10" : "bg-white/40 border-transparent hover:bg-white hover:shadow-sm hover:border-gray-200"}`,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start justify-between mb-2", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0 pr-2", children: [
                  ...
                  task.needs_review === 1 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-1 mb-1 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-[10px] text-amber-700 font-medium", children: t2("taskList.needsReview") }),
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-col gap-1", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    className: `relative inline-flex h-4 w-7 items-center rounded-full transition-colors ${isEnabled ? "bg-blue-600" : "bg-gray-300"}`,
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "span",
                      {
                        className: `inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${isEnabled ? "translate-x-3.5" : "translate-x-0.5"}`
                      }
                    )
                  }
                ) })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `absolute right-2 bottom-2 flex items-center gap-1 transition-opacity ${isSelected || runningTasks.has(task.id) ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`, children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    title: t2("taskList.runNow"),
                    children: runningTasks.has(task.id) ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "animate-spin rounded-full h-3.5 w-3.5 border-2 border-blue-600 border-t-transparent" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { className: "w-3.5 h-3.5 fill-current" })
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    onClick: (e) => handleDelete2(e, task.id),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 bg-gray-50/50 rounded-2xl border border-gray-100 overflow-hidden flex flex-col relative", children: showTaskForm ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      TaskForm,
      {
        task: editingTask,
        onClose: onCloseForm,
    ...
    ) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 bg-white rounded-2xl border border-gray-100 shadow-sm flex items-center justify-center mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { className: "w-8 h-8 text-gray-300" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-gray-900 font-medium mb-1", children: t2("taskList.noTaskSelected") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm max-w-xs mx-auto", children: t2("taskList.noTaskSelectedDesc") }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          onClick: onNewTask,
...
function name(name2, options) {
  const settings2 = emptyOptions$3;
...
function toString$1(value, options) {
  const settings2 = emptyOptions$2;
...
function createState(tree, options) {
  ...
  const handlers$1 = { ...handlers, ...settings2.handlers };
  const state = {
    all: all2,
    applyData,
    definitionById,
...
function markdownTable(table2, options) {
  ...
  while (++columnIndex < mostCellsPerRow) {
    ...
    let size = settings2.alignDelimiters === false ? 1 : Math.max(
      1,
  ...
  while (++rowIndex < cellMatrix.length) {
    ...
    while (++columnIndex < mostCellsPerRow) {
      ...
      if (settings2.padding !== false && // Don’t add the opening space if we’re not aligning and the cell is
      // empty: there will be a closing space.
      !(settings2.alignDelimiters === false && cell === "") && (settings2.delimiterStart !== false || columnIndex)) {
        line.push(" ");
      }
      if (settings2.alignDelimiters !== false) {
        line.push(before);
...
function ExecutionLog() {
  ...
  const [selectedLogId, setSelectedLogId] = reactExports.useState(null);
  const [checkedLogIds, setCheckedLogIds] = reactExports.useState(/* @__PURE__ */ new Set());
  reactExports.useEffect(() => {
    if (logs.length > 0 && !selectedLogId) {
      setSelectedLogId(logs[0].id);
    }
  }, [logs, selectedLogId]);
  const selectedLog = logs.find((l2) => l2.id === selectedLogId) || null;
  const handleCheck = (id2, checked) => {
    const newChecked = new Set(checkedLogIds);
    if (checked) {
      newChecked.add(id2);
    } else {
      newChecked.delete(id2);
    }
    setCheckedLogIds(newChecked);
  };
  const handleSelectAll = (checked) => {
    if (checked) {
      setCheckedLogIds(new Set(logs.map((l2) => l2.id)));
    } else {
      setCheckedLogIds(/* @__PURE__ */ new Set());
    }
  };
  const handleDeleteSelected = async () => {
    if (checkedLogIds.size === 0) return;
    if (!confirm(t2("executionLog.confirmDelete", { count: checkedLogIds.size }))) return;
    ...
    if (selectedLogId && idsToDelete.includes(selectedLogId)) {
      const remainingLogs = logs.filter((l2) => !idsToDelete.includes(l2.id));
  ...
  if (loading) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-full flex items-center justify-center bg-white rounded-tl-2xl", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" }) });
  }
  if (error) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-full bg-white rounded-tl-2xl p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4 text-red-700", children: error }) });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-full gap-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-80 flex-shrink-0 flex flex-col gap-1", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-4 px-2 flex items-center justify-between", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-lg font-bold text-gray-900", children: t2("executionLog.listTitle") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mt-1", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-400", children: t2("executionLog.selectAll") })
          ] })
        ] }),
        checkedLogIds.size > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: t2("executionLog.deleteSelected", { count: checkedLogIds.size })
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto pr-2 space-y-2", children: logs.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center py-12 px-4 border-2 border-dashed border-gray-200 rounded-xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-8 h-8 text-gray-300 mx-auto mb-2", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("executionLog.noLogsFound") })
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: logs.map((log) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        LogListItem,
        {
          log,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 bg-gray-50/50 rounded-2xl border border-gray-100 shadow-sm flex flex-col overflow-hidden min-w-0", children: selectedLog ? /* @__PURE__ */ jsxRuntimeExports.jsx(LogDetail, { log: selectedLog }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 bg-white rounded-2xl border border-gray-100 shadow-sm flex items-center justify-center mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-8 h-8 text-gray-300", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-gray-900 font-medium mb-1", children: t2("executionLog.noLogSelected") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm max-w-xs mx-auto", children: t2("executionLog.noLogSelectedDesc") })
    ] }) })
  ] });
}
function LogListItem({ log, isSelected, isChecked, onCheck, onClick }) {
  const { t: t2 } = useTranslation();
  const statusColors = {
    running: "bg-blue-500",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    ...
    {
      onClick,
      className: `group w-full flex items-center p-3 rounded-xl transition-all border cursor-pointer relative ${isSelected ? "bg-white shadow-md border-blue-200 ring-1 ring-blue-100 z-10" : "bg-white/40 border-transparent hover:bg-white hover:shadow-sm hover:border-gray-200"}`,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${statusColors[log.status]}`, children: log.status === "running" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "flex h-2 w-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "animate-ping absolute inline-flex h-2 w-2 rounded-full bg-blue-400 opacity-75" }) }) }),
...
function LogDetail({ log: initialLog }) {
  ...
  const extractCurrentActivity = (output) => {
    if (!output || output.trim().length === 0) {
      return t2("executionLog.activity.initializing");
    }
    ...
    if (willMatch) {
      ...
      const actionMap = {
        "search": t2("executionLog.activity.searching"),
        "fetch": t2("executionLog.activity.fetching"),
        "analyze": t2("executionLog.activity.analyzing"),
        "access": t2("executionLog.activity.accessing"),
        "check": t2("executionLog.activity.checking"),
        "list": t2("executionLog.activity.listing"),
        "get": t2("executionLog.activity.getting"),
        "read": t2("executionLog.activity.reading"),
        "process": t2("executionLog.activity.processingAction"),
        "execute": t2("executionLog.activity.executing"),
        "connect": t2("executionLog.activity.connecting"),
        "query": t2("executionLog.activity.querying"),
        "calculate": t2("executionLog.activity.calculating"),
        "generate": t2("executionLog.activity.generating"),
        "create": t2("executionLog.activity.creating"),
        "update": t2("executionLog.activity.updating"),
        "retrieve": t2("executionLog.activity.retrieving"),
        "confirm": t2("executionLog.activity.confirming"),
        "identify": t2("executionLog.activity.identifying")
      };
      const objectPatterns = [
        { pattern: /ga4\s+(schema|metadata|data|dimension|metric)/i, label: "GA4" },
        { pattern: /the\s+ga4\s+(schema|metadata)/i, label: t2("executionLog.activity.ga4Schema") },
        { pattern: /performance\s+data/i, label: t2("executionLog.activity.performanceData") },
        { pattern: /(dimension|metric)\s+names?/i, label: t2("executionLog.activity.dimensionsMetrics") },
        { pattern: /high-traffic\s+articles?/i, label: t2("executionLog.activity.highTrafficArticles") },
        { pattern: /engagement\s+(data|metrics?)/i, label: t2("executionLog.activity.engagementData") },
        { pattern: /bounce\s+rates?/i, label: t2("executionLog.activity.bounceRate") }
      ];
      ...
      return t2("executionLog.activity.executingAction", { action: shortAction });
    }
    ...
    if (ingMatch) {
      ...
      const actionMap = {
        "searching": t2("executionLog.activity.searching"),
        "fetching": t2("executionLog.activity.fetching"),
        "analyzing": t2("executionLog.activity.analyzing"),
        "accessing": t2("executionLog.activity.accessing"),
        "checking": t2("executionLog.activity.checking"),
        "processing": t2("executionLog.activity.processingAction"),
        "executing": t2("executionLog.activity.executing"),
        "connecting": t2("executionLog.activity.connecting"),
        "querying": t2("executionLog.activity.querying"),
        "calculating": t2("executionLog.activity.calculating"),
        "generating": t2("executionLog.activity.generating"),
        "creating": t2("executionLog.activity.creating"),
        "updating": t2("executionLog.activity.updating"),
        "retrieving": t2("executionLog.activity.retrieving"),
        "loading": t2("executionLog.activity.loading"),
        "reading": t2("executionLog.activity.reading")
      };
      const translatedAction = actionMap[action.toLowerCase()] || t2("executionLog.activity.actionGeneric", { verb: action });
    ...
    if (recentText.includes("ga4") || recentText.includes("google analytics")) {
      if (recentText.includes("schema") || recentText.includes("metadata")) {
        return t2("executionLog.activity.checkingGA4Schema");
      }
      if (recentText.includes("fetch") || recentText.includes("get") || recentText.includes("retrieve")) {
        return t2("executionLog.activity.fetchingGA4Data");
      }
      if (recentText.includes("analyze") || recentText.includes("analysis")) {
        return t2("executionLog.activity.analyzingGA4Data");
      }
      return t2("executionLog.activity.processingGA4");
    }
    if (recentText.includes("mcp") || recentText.includes("tool")) {
      return t2("executionLog.activity.callingMCPTool");
    }
    if (recentText.includes("permission") || recentText.includes("授權") || recentText.includes("權限")) {
      return t2("executionLog.activity.processingPermission");
    }
    if (lastLine.length > 50) {
      return t2("executionLog.activity.processingText", { text: lastLine.substring(0, 50) });
    }
    return t2("executionLog.activity.processing");
  };
  const currentActivity = log.status === "running" && log.output ? extractCurrentActivity(log.output) : log.status === "running" ? t2("executionLog.activity.initializingTask") : "";
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto overflow-x-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-6 py-4 space-y-4 min-w-0", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-6 text-sm text-gray-500", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-3.5 h-3.5", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDateTime(log.started_at) })
        ] }),
        log.finished_at && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1.5", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-3.5 h-3.5", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M13 10V3L4 14h7v7l9-11h-7z" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDuration(log.started_at, log.finished_at) })
        ] })
      ] }),
      log.status === "running" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 text-blue-600 bg-blue-50 px-4 py-2.5 rounded-lg", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm font-medium", children: currentActivity || t2("executionLog.taskRunning") })
      ] }),
      log.error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `rounded-lg p-4 ${log.error.includes("🚫") || log.error.includes("安全檢查") ? "bg-red-100 border-2 border-red-400 shadow-lg" : "bg-red-50 border border-red-200"}`, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 mb-2", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `font-medium ${log.error.includes("🚫") || log.error.includes("安全檢查") ? "text-red-800 text-base" : "text-red-700 text-sm"}`, children: log.error.includes("🚫") || log.error.includes("安全檢查") ? t2("executionLog.securityCheckFailed") : t2("executionLog.errorLabel") })
        ] }),
        ...
        (log.error.includes("🚫") || log.error.includes("安全檢查")) && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-red-200 border-2 border-red-400 rounded-lg p-4 mt-3", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "text-sm font-bold text-red-900 mb-2", children: t2("executionLog.securityWarning.title") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-red-800 mb-2", children: t2("executionLog.securityWarning.desc") }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-red-300 rounded p-2 mt-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-semibold text-red-900 mb-1", children: t2("executionLog.securityWarning.measuresTitle") }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("ul", { className: "text-xs text-red-800 list-disc list-inside space-y-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.securityWarning.measure1") }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.securityWarning.measure2") }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.securityWarning.measure3") })
              ] })
            ] })
          ] })
        ] }) }),
        !log.error.includes("🚫") && !log.error.includes("安全檢查") && (log.error.includes("Denied by policy") || log.error.includes("政策拒絕") || log.error.includes("系統政策")) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-red-100 border border-red-300 rounded p-3 text-xs text-red-800 mt-3", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium mb-2", children: t2("executionLog.policyDenied.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("ol", { className: "list-decimal list-inside space-y-1", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.policyDenied.step1") }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { children: [
              t2("executionLog.policyDenied.step2pre"),
              /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-red-200 px-1 rounded", children: '"trust": true' }),
              t2("executionLog.policyDenied.step2post")
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.policyDenied.step3") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.policyDenied.step4") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.policyDenied.step5") })
          ] })
        ] })
      ] }),
      log.output && (log.output.includes("Denied by policy") || log.output.includes("操作遭到系統政策拒絕") || log.output.includes("政策拒絕")) && !log.error && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bg-red-50 border border-red-200 rounded-lg p-4 mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "text-sm font-semibold text-red-900 mb-2", children: t2("executionLog.mcpDenied.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-red-100 border border-red-300 rounded p-3 text-xs text-red-800 mb-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-medium mb-2", children: t2("executionLog.mcpDenied.stepsTitle") }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("ol", { className: "list-decimal list-inside space-y-1", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.mcpDenied.step1") }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { children: [
                t2("executionLog.mcpDenied.step2pre"),
                /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-red-200 px-1 rounded", children: '"trust": true' }),
                t2("executionLog.mcpDenied.step2post")
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.mcpDenied.step3") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("executionLog.mcpDenied.step4") })
            ] })
          ] })
        ] })
      ] }) }),
      ...
      ] }) : log.status === "running" ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-gray-400 text-sm", children: t2("executionLog.waitingForOutput") }) : null
    ] }) })
  ] });
}
...
function formatRelativeTime(isoString, t2) {
  const date = new Date(isoString);
  ...
  if (diffMins < 1) return t2("executionLog.time.justNow");
  if (diffMins < 60) return t2("executionLog.time.minutesAgo", { count: diffMins });
  if (diffHours < 24) return t2("executionLog.time.hoursAgo", { count: diffHours });
  if (diffDays < 7) return t2("executionLog.time.daysAgo", { count: diffDays });
...
function formatDuration(start, end) {
  ...
  const seconds = Math.round(diffMs % 6e4 / 1e3);
...
const main$1 = {
  ...
  retry: {
    networkError: "⏳ Network error, retrying in {{delay}}s (attempt {{attempt}}/{{max}})...\nError: {{error}}",
    exhausted: "⚠️ Failed after {{max}} retries"
  },
  security: {
    unknownCommand: "unknown",
    promptBlocked: "🚫 Security check failed: {{reason}}\n\nTo protect your system, execution of the command containing dangerous delete operations has been blocked.\nDetected command: {{command}}\n\nTo perform this operation, please explicitly authorize and confirm the risk.",
    runtimeBlocked: "🚫 Security check failed: {{reason}}\n\nTo protect your system, execution has been automatically stopped.\nDetected command: {{command}}\n\nStrictly prohibited: actively deleting items without user authorization."
  },
  dbError: {
    title: "Orbit Agents failed to start",
    body: "The local database could not be opened, so the app cannot continue.\n\nThis usually means the bundled database engine (better-sqlite3) was built for a different CPU architecture or Electron version.\n\nDetails:\n{{details}}"
  }
};
const settings$1 = {
  title: "Settings",
  subtitle: "Manage app preferences and connections",
  language: {
    label: "Language",
  ...
  general: {
    ...
    description: "Configure how Orbit Agents behaves on your system.",
    launchAtLogin: {
      label: "Launch at Login",
      description: "Automatically start the application when you log in."
    },
    autoUpdate: {
      label: "Auto Update",
      description: "Automatically check for and notify about updates."
    },
    softwareUpdate: {
  ...
  update: {
    ...
    methodAsar: "A lightweight update is ready for quick download.",
    methodFull: "A new version can be downloaded and installed.",
    manualDownload: "Manual Download",
    download: "Download Update",
    downloading: "Downloading update...",
    progress: "{{percent}}% complete",
    downloaded: "Update downloaded!",
    restartNote: "The app will restart to install the update.",
  ...
  cli: {
    ...
    defaultPathNote: "Leave empty to use the default system path.",
    unknownError: "Unknown error"
  },
  claude: {
    title: "Claude CLI",
    description: "Configure the connection to Anthropic's Claude command line tool.",
    pathNotePrefix: "Leave empty to use the default system path. Windows users should use",
    pathNoteSuffix: "instead of the full path to cli.js."
  },
  codex: {
    title: "Codex CLI",
  ...
  antigravity: {
    ...
    description: "Configure the connection to the Antigravity command line tool.",
    placeholder: "Use default (agy in PATH)"
  },
  email: {
    title: "Email (SMTP)",
    description: "Configure email settings for sending task notifications.",
    smtpHost: "SMTP Host",
    port: "Port",
    username: "Username",
    password: "Password",
    ...
    fromNote: "Optional. Default depends on your SMTP provider.",
    testConfig: "Test Configuration",
    sendTest: "Send Test",
    missingAddress: "Please enter an email address",
...
const taskList$1 = {
  ...
  needsReview: "Needs review: Gemini removed, reconfigure provider",
  runNow: "Run Now",
  noTaskSelected: "No Task Selected",
  noTaskSelectedDesc: "Select a task from the list to view details or create a new one.",
  createNewTask: "Create New Task"
};
const taskForm$1 = {
  editTitle: "Edit Task",
  newTitle: "New Task",
  subtitle: "Configure your scheduled AI task",
  optional: "(Optional)",
  name: {
    label: "Task Name",
  ...
  schedule: {
    ...
    exampleDailyDesc: "(daily 9am)",
    exampleIntervalDesc: "(every 15 min)"
  },
  skills: {
    label: "Skills",
  ...
  knowledge: {
    label: "Knowledge",
    description: "Auto-record reusable insights after each run",
...
const schedule$1 = {
  ...
  desc: {
    everyN: "Every {{value}} {{unit}}",
    dailyAt: "Daily at {{time}}",
    weekly: "Every {{days}} at {{time}}",
    weeklyEveryN: "Every {{n}} weeks on {{days}} at {{time}}",
...
const welcome$1 = {
  slides: {
    alt0: "Orbit Agents — task scheduling interface",
    alt1: "Orbit Agents — execution log with AI output"
  },
  nav: {
    download: "Download"
  },
  hero: {
    ...
    subtitle: "Schedule Claude, Codex, or Antigravity to run automatically on your desktop. No servers, no cloud fees, no node wiring — just write a prompt, pick a schedule, and let it work for you.",
    downloadMac: "Download for Mac",
    viewGitHub: "View on GitHub",
    downloadWindows: "Windows",
    downloadPortable: "Portable (.zip)"
  },
  screenshots: {
  ...
  steps: {
    sectionTitle: "Three steps. That's it.",
    sectionSubtitle: "No complex setup, no steep learning curve.",
    pickAI: {
      title: "Pick your AI",
      desc: "Claude, Codex, or Antigravity — choose the model that fits your task."
    },
    writePrompt: {
      title: "Write your prompt",
      desc: "Describe what you want done in plain language, or use a slash command — no node wiring needed."
    },
    setSchedule: {
      title: "Set a schedule",
      desc: "Daily, weekly, or custom cron — Orbit runs it on time, every time."
    }
  },
  comparison: {
    sectionTitle: "Why Orbit?",
    sectionSubtitle: "See how Orbit compares to traditional automation platforms.",
    featureColLabel: "Feature",
    othersHeader: "n8n / Zapier / Others",
    cost: {
      label: "Cost",
      others: "Cloud subscription fees",
      orbit: "Free, runs locally"
    },
    setup: {
      label: "Setup",
      others: "Wire nodes & connectors",
      orbit: "Write a prompt"
    },
    aiIntegration: {
      label: "AI integration",
      others: "Requires add-ons",
      orbit: "Claude, Codex & Antigravity built-in"
    },
    data: {
      label: "Data",
      others: "Stored in the cloud",
      orbit: "100% on your machine"
    },
    platform: {
      label: "Platform",
      others: "Browser-based / macOS only",
      orbit: "macOS + Windows"
    },
    sourceCode: {
      label: "Source code",
      others: "Closed / limited",
      orbit: "Open source (MIT)"
    }
  },
  features: {
    sectionTitle: "Key Features",
    sectionSubtitle: "Everything you need to automate AI tasks on your desktop.",
    smartScheduling: {
      title: "Smart Scheduling",
      desc: "Interval, daily, weekly, monthly, or raw cron expressions — pick what works for you."
    },
    deepAI: {
      title: "Deep AI Integration",
      desc: "First-class support for Claude, Codex, and Antigravity CLI, with MCP server connections."
    },
    executionLogs: {
      title: "Execution Logs",
      desc: "Full visibility into every run — see what the agent did, step by step."
    },
    emailNotifications: {
      title: "Email Notifications",
      desc: "Get notified when tasks complete or fail — never miss a result."
    },
    crossPlatform: {
      title: "Cross-Platform",
      desc: "Native desktop app for macOS and Windows. No browser tab required."
    },
    autoRetry: {
      title: "Auto-Retry on Failure",
      desc: "Network hiccup? Orbit retries automatically so your scheduled tasks stay on track."
    }
  },
  installation: {
    sectionTitle: "Installation",
    sectionSubtitle: "Download, install, and you're ready to go.",
    macos: {
      title: "macOS",
      desc: "Since Orbit Agents doesn't have an Apple Developer certificate yet, macOS Gatekeeper may block the app. Fix it with either method:",
      option1Title: "Option 1: Terminal command (Recommended)",
      option2Title: "Option 2: Right-click to open",
      option2Step1: "Right-click (or Control-click) the app in Finder",
      option2Step2: 'Select "Open"',
      option2Step3: 'Click "Open" again in the dialog',
      note: "Only needed the first time — after that, the app opens normally."
    },
    windows: {
      title: "Windows",
      desc: "Windows SmartScreen may show a warning for unsigned apps. Click through to proceed:",
      step1: 'Click "More info" on the SmartScreen popup',
      step2: 'Click "Run anyway"',
      note: "Or use the portable .zip version — no installer, no SmartScreen."
    }
  },
  cta: {
    title: "Ready to put your AI on autopilot?",
    subtitle: "Free, open source, no signup required.",
    downloadMac: "Download for Mac",
    downloadWindows: "Download for Windows"
  },
  footer: {
    creditPre: "MIT License · Made by",
    creditPost: "",
    download: "Download"
  }
};
const executionLog$1 = {
  listTitle: "Execution Logs",
  selectAll: "Select all",
  deleteSelected: "Delete ({{count}})",
  confirmDelete: "Are you sure you want to delete {{count}} logs?",
  noLogsFound: "No logs found.",
  noLogSelected: "No Log Selected",
  noLogSelectedDesc: "Select an execution log from the list to view its details and output.",
  ...
  securityWarning: {
    title: "⚠️ Security Protection Activated",
    desc: "System detected an attempt to execute a dangerous delete operation. Execution was automatically stopped to protect your system.",
    measuresTitle: "Protection measures:",
    measure1: "Strictly prohibit actively deleting items without user authorization",
    measure2: "Automatically detect and block dangerous delete commands (e.g. rm -rf)",
    measure3: "Execution was immediately stopped with no impact to the system"
  },
  policyDenied: {
    title: "🔧 MCP Tool Policy Denied - Resolution Steps:",
    step1: "Check MCP settings in CLI configuration file",
    step2pre: "Add ",
    step2post: " to your MCP server configuration",
    step3: "Confirm MCP Server is running correctly",
    step4: "Confirm API permissions are properly configured",
    step5: "Restart CLI or the application"
  },
  mcpDenied: {
    title: "MCP Tool Denied by System Policy",
    stepsTitle: "🔧 Resolution Steps:",
    step1: "Check MCP settings in CLI configuration file",
    step2pre: "Add ",
    step2post: " to MCP server configuration",
  ...
  activity: {
    ...
    loading: "Loading",
    ga4Schema: "GA4 schema",
    performanceData: "performance data",
    dimensionsMetrics: "dimensions and metrics",
    highTrafficArticles: "high-traffic articles",
    engagementData: "engagement data",
    bounceRate: "bounce rate",
    checkingGA4Schema: "Checking GA4 schema...",
    fetchingGA4Data: "Fetching GA4 data...",
    analyzingGA4Data: "Analyzing GA4 data...",
    processingGA4: "Processing GA4 operations...",
    callingMCPTool: "Calling MCP tool...",
    processingPermission: "Processing permission request..."
  }
};
const en = {
  common: common$1,
...
const main = {
  ...
  retry: {
    networkError: "⏳ 網路錯誤，{{delay}} 秒後自動重試（第 {{attempt}}/{{max}} 次）...\n錯誤: {{error}}",
    exhausted: "⚠️ 已重試 {{max}} 次仍然失敗"
  },
  security: {
    unknownCommand: "未知",
    promptBlocked: "🚫 安全檢查失敗: {{reason}}\n\n為了保護您的系統安全，已阻止執行包含危險刪除操作的命令。\n檢測到的命令: {{command}}\n\n如需執行此操作，請明確授權並確認風險。",
    runtimeBlocked: "🚫 安全檢查失敗: {{reason}}\n\n為了保護您的系統安全，已自動停止執行。\n檢測到的命令: {{command}}\n\n嚴格禁止在未經使用者授權下主動刪除項目。"
  },
  dbError: {
    title: "Orbit Agents 無法啟動",
    body: "本機資料庫無法開啟，應用程式無法繼續執行。\n\n這通常代表內建的資料庫引擎（better-sqlite3）是針對不同 CPU 架構或 Electron 版本所建置。\n\n詳細資訊：\n{{details}}"
  }
};
const settings = {
  title: "設定",
  subtitle: "管理應用程式偏好設定與連線",
  language: {
    label: "語言",
  ...
  nav: {
    ...
    claude: {
      label: "Claude CLI",
      desc: "Anthropic Claude 設定"
    },
    codex: {
      label: "Codex CLI",
      desc: "OpenAI Codex 設定"
    },
    antigravity: {
      label: "Antigravity CLI",
      desc: "Antigravity 設定"
  ...
  general: {
    title: "一般設定",
    description: "設定 Orbit Agents 在您系統上的行為",
    launchAtLogin: {
      label: "登入時啟動",
      description: "登入系統時自動啟動應用程式"
    },
    autoUpdate: {
      label: "自動更新",
      description: "自動檢查並通知有可用更新"
    },
    softwareUpdate: {
      label: "軟體更新",
      description: "檢查是否有新版本可用"
    },
    checkForUpdates: "檢查更新"
  },
  update: {
    upToDate: "您已使用最新版本！",
    available: "{{version}} 版本現已推出！",
    methodAsar: "輕量更新已備妥，可快速下載",
    methodFull: "新版本可供下載並安裝",
    manualDownload: "手動下載",
    download: "下載更新",
    downloading: "正在下載更新...",
    progress: "{{percent}}% 完成",
    downloaded: "更新已下載！",
    restartNote: "應用程式將重新啟動以安裝更新",
    restartInstall: "重新啟動並安裝"
  },
  cli: {
    pathLabel: "CLI 路徑",
    testConnection: "測試連線",
    ...
    defaultPathNote: "留空使用系統預設路徑",
    unknownError: "未知錯誤"
  },
  claude: {
    title: "Claude CLI",
    description: "設定與 Anthropic Claude 命令列工具的連線",
    pathNotePrefix: "留空使用系統預設路徑，Windows 使用者請使用",
    pathNoteSuffix: "取代完整的 cli.js 路徑"
  },
  codex: {
    title: "Codex CLI",
  ...
  antigravity: {
    title: "Antigravity CLI",
    description: "設定與 Antigravity 命令列工具的連線",
    placeholder: "使用預設（PATH 中的 agy）"
  },
  email: {
    title: "電子郵件（SMTP）",
    description: "設定傳送任務通知的電子郵件",
    smtpHost: "SMTP 主機",
    port: "連接埠",
    username: "使用者名稱",
    password: "密碼",
    passwordPlaceholder: "應用程式密碼",
    fromAddress: "寄件者地址",
    fromNote: "選填，預設值由您的 SMTP 服務提供者決定",
    testConfig: "測試設定",
    sendTest: "傳送測試信",
    missingAddress: "請輸入電子郵件地址",
    testSuccess: "測試信件已成功傳送！",
    testFailed: "測試信件傳送失敗"
  }
};
const app = {
...
const taskList = {
  ...
  emptyState: "目前沒有任務",
  createOne: "建立一個",
  needsReview: "需要確認：Gemini 已移除，請重新設定供應商",
  runNow: "立即執行",
  noTaskSelected: "未選取任務",
  noTaskSelectedDesc: "從列表選取任務以查看詳情，或建立新任務",
  createNewTask: "建立新任務"
};
const taskForm = {
  editTitle: "編輯任務",
  newTitle: "新增任務",
  subtitle: "設定您的排程 AI 任務",
  optional: "（選填）",
  name: {
    label: "任務名稱",
    placeholder: "例：每日安全報告"
  },
  description: {
    label: "說明",
    placeholder: "簡短描述此任務"
  ...
  schedule: {
    ...
    week: "週",
    weeks: "週",
    cronFormat: "Cron 格式：分 時 日 月 星期",
    examplesLabel: "範例：",
    exampleDailyDesc: "（每天早上 9 點）",
    exampleIntervalDesc: "（每 15 分鐘）"
  },
  skills: {
    label: "技能",
    changeProject: "更換專案",
...
const schedule = {
  ...
  desc: {
    everyN: "每隔 {{value}} {{unit}}",
    dailyAt: "每天 {{time}}",
    weekly: "每週{{days}} {{time}}",
    weeklyEveryN: "每 {{n}} 週的{{days}} {{time}}",
    monthly: "每月第 {{day}} 天 {{time}}",
    daySeparator: "、"
  }
};
const welcome = {
  slides: {
    alt0: "Orbit Agents 任務排程介面",
    alt1: "Orbit Agents AI 輸出執行記錄"
  },
  nav: {
    download: "下載"
  },
  hero: {
    badge: "免費 & 開源",
    title: "你的 AI 任務，",
    titleHighlight: "全都自動化。",
    subtitle: "讓 Claude、Codex 或 Antigravity 自動在你的電腦上執行。不用伺服器、不用月費、不用拉節點，只要寫提示詞、設排程，剩下的交給它。",
  ...
  steps: {
    sectionTitle: "三個步驟，就這樣。",
    sectionSubtitle: "不需複雜設定，不需要花時間摸索。",
    pickAI: {
      title: "選擇你的 AI",
      desc: "Claude、Codex 或 Antigravity，選一個最適合你任務的模型。"
    },
    writePrompt: {
      title: "寫下提示詞",
      desc: "用自然語言描述你想完成的事，或使用斜線指令，不需要拉線接節點。"
    },
    setSchedule: {
      title: "設定排程",
      desc: "每天、每週或自訂 cron 表達式，Orbit 準時執行，每次都不漏。"
    }
  },
  comparison: {
    sectionTitle: "為什麼選 Orbit？",
    sectionSubtitle: "看看 Orbit 和傳統自動化平台有什麼不同。",
    featureColLabel: "功能",
    othersHeader: "n8n / Zapier / 其他",
    cost: {
      label: "費用",
      others: "雲端訂閱費",
      orbit: "免費，在本機執行"
    ...
    aiIntegration: {
      label: "AI 整合",
      others: "需要額外擴充",
      orbit: "Claude、Codex 與 Antigravity 內建支援"
    },
    data: {
      label: "資料",
      others: "儲存在雲端",
  ...
  features: {
    sectionTitle: "核心功能",
    sectionSubtitle: "讓桌面 AI 任務全面自動化的必備功能。",
    smartScheduling: {
      title: "智慧排程",
      desc: "間隔、每天、每週、每月或直接寫 cron 表達式，隨你選擇。"
    },
    deepAI: {
      title: "深度 AI 整合",
      desc: "原生支援 Claude、Codex 與 Antigravity CLI，並可連接 MCP 伺服器。"
    },
    executionLogs: {
      title: "執行記錄",
      desc: "完整呈現每次執行的過程，逐步追蹤 AI 做了什麼。"
    },
    emailNotifications: {
      title: "電子郵件通知",
      desc: "任務完成或失敗時立即通知，不錯過任何結果。"
    },
    crossPlatform: {
      title: "跨平台支援",
      desc: "macOS 與 Windows 原生桌面應用程式，不需要開瀏覽器。"
    },
    autoRetry: {
      title: "失敗自動重試",
      desc: "網路不穩？Orbit 會自動重試，讓排程任務持續正常執行。"
    }
  },
  installation: {
    sectionTitle: "安裝說明",
    sectionSubtitle: "下載、安裝，就可以開始使用了。",
    macos: {
      title: "macOS",
      desc: "由於 Orbit Agents 尚未取得 Apple 開發者憑證，macOS Gatekeeper 可能會阻擋應用程式。用以下任一方式解決：",
      option1Title: "方法一：在終端機執行指令（推薦）",
      option2Title: "方法二：右鍵點擊開啟",
      option2Step1: "在 Finder 中右鍵點擊（或 Control + 點擊）應用程式",
      option2Step2: "選擇「開啟」",
      option2Step3: "在對話框中再次點擊「開啟」",
      note: "只有第一次需要這樣做，之後就可以直接開啟。"
    },
    windows: {
      title: "Windows",
      desc: "Windows SmartScreen 可能會對未簽署的應用程式顯示警告，點選繼續即可：",
      step1: "在 SmartScreen 彈出視窗中點「更多資訊」",
      step2: "點「仍要執行」",
      note: "或使用 Portable .zip 版本，免安裝、免 SmartScreen。"
    }
  },
  cta: {
    title: "準備好讓你的 AI 自動執行了嗎？",
    subtitle: "免費、開源，不需要註冊。",
    downloadMac: "下載 Mac 版",
    downloadWindows: "下載 Windows 版"
  },
  footer: {
    creditPre: "MIT License · 由",
    creditPost: " 製作",
    download: "下載"
  }
};
const executionLog = {
  ...
  confirmDelete: "確定要刪除 {{count}} 筆記錄？",
  noLogsFound: "暫無執行記錄",
  noLogSelected: "未選取記錄",
  noLogSelectedDesc: "從列表選取一筆執行記錄，查看詳情與輸出",
  unknownTask: "未知任務",
  copy: "複製",
  copied: "已複製！",
  taskRunning: "任務執行中...",
  ...
  securityWarning: {
    title: "⚠️ 安全保護機制已啟動",
    desc: "系統檢測到嘗試執行危險的刪除操作，已自動停止執行以保護您的系統安全。",
    measuresTitle: "保護措施：",
    measure1: "嚴格禁止在未經使用者授權下主動刪除項目",
    measure2: "自動檢測並阻止危險的刪除命令（如 rm -rf）",
    measure3: "執行已立即停止，不會對系統造成任何影響"
  },
  policyDenied: {
    title: "🔧 MCP 工具政策拒絕 - 解決步驟：",
    step1: "檢查 CLI 設定檔中的 MCP 設定",
    step2pre: "為您的 MCP server 新增 ",
    step2post: " 設定",
    step3: "確認 MCP Server 已正確啟動",
    step4: "確認 API 權限已正確配置",
    step5: "重新啟動 CLI 或應用程式"
  },
  mcpDenied: {
    title: "MCP 工具被系統政策拒絕",
    stepsTitle: "🔧 解決步驟：",
    step1: "檢查 CLI 設定檔中的 MCP 設定",
    step2pre: "為 MCP server 新增 ",
    step2post: "",
  ...
  activity: {
    initializing: "正在初始化...",
    initializingTask: "正在初始化任務...",
    processing: "正在處理中...",
    processingText: "正在處理: {{text}}...",
    executingAction: "正在執行: {{action}}...",
    actionGeneric: "正在{{verb}}",
    searching: "正在搜尋",
    fetching: "正在獲取",
    analyzing: "正在分析",
    accessing: "正在存取",
    ...
    loading: "正在載入",
    ga4Schema: "GA4 結構",
    performanceData: "效能數據",
    dimensionsMetrics: "維度和指標",
    highTrafficArticles: "高流量文章",
    engagementData: "互動數據",
    bounceRate: "跳出率",
    checkingGA4Schema: "正在檢查 GA4 結構...",
    fetchingGA4Data: "正在獲取 GA4 數據...",
    analyzingGA4Data: "正在分析 GA4 數據...",
    processingGA4: "正在處理 GA4 相關操作...",
    callingMCPTool: "正在呼叫 MCP 工具...",
    processingPermission: "正在處理權限請求..."
  }
};
const zhTW = {
  common,
...
function resolveLocale(pref, systemLocale) {
  ...
  const lc2 = (systemLocale || "").toLowerCase();
  if (lc2.startsWith("zh")) return "zh-TW";
  return "en";
}
...
function applyLanguagePreference(pref) {
  instance.changeLanguage(resolveLocale(pref, navigator.language));
}
function Settings({}) {
  const { t: t2 } = useTranslation();
  ...
  reactExports.useEffect(() => {
    if (!loading && settings2 && !isLoaded) {
      setFormData({
        email_smtp_host: settings2.email_smtp_host || "",
        email_smtp_port: settings2.email_smtp_port || "587",
        email_smtp_user: settings2.email_smtp_user || "",
        email_smtp_pass: settings2.email_smtp_pass || "",
        email_from: settings2.email_from || "",
        claude_cli_path: settings2.claude_cli_path || "",
        codex_cli_path: settings2.codex_cli_path || "",
        antigravity_cli_path: settings2.antigravity_cli_path || "",
        auto_launch: settings2.auto_launch ?? "true",
        auto_update: settings2.auto_update ?? "true"
      });
      setLanguage(settings2.language ?? "system");
      setIsLoaded(true);
    }
  }, [loading, settings2, isLoaded]);
  ...
  const performSave = reactExports.useCallback(async () => {
    setIsSaving(true);
    try {
  ...
  const handleTestClaude = async () => {
    ...
    try {
      ...
      setClaudeResult({
        success: false,
        output: "",
        error: err instanceof Error ? err.message : t2("settings.cli.unknownError")
      });
    } finally {
  ...
  const handleTestEmail = async () => {
    if (!testEmailAddress) {
      setEmailResult({ success: false, message: t2("settings.email.missingAddress") });
      return;
    }
    ...
    try {
      ...
      setEmailResult({ success: true, message: t2("settings.email.testSuccess") });
    } catch (err) {
      setEmailResult({
        success: false,
        message: err instanceof Error ? err.message : t2("settings.email.testFailed")
      });
    } finally {
  ...
  if (loading && !isLoaded) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center justify-center h-full", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-2 border-blue-600 border-t-transparent" }) });
  }
  function NavButton({ tab: tab2, icon: Icon2, label, desc }) {
    const isActive = activeTab === tab2;
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "button",
      {
        onClick: () => setActiveTab(tab2),
        className: `w-full text-left px-4 py-3 rounded-xl transition-all flex items-start gap-3 group ${isActive ? "bg-white shadow-sm ring-1 ring-gray-200" : "hover:bg-white/50"}`,
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `p-2 rounded-lg transition-colors ${isActive ? "bg-blue-50 text-blue-600" : "bg-gray-100 text-gray-500 group-hover:bg-white group-hover:text-gray-600"}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Icon2, { className: "w-5 h-5" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `font-semibold text-sm ${isActive ? "text-gray-900" : "text-gray-600"}`, children: label }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs text-gray-400 mt-0.5 font-normal", children: desc })
          ] })
        ]
      }
    );
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-full gap-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-64 flex-shrink-0 flex flex-col gap-1", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mb-6 px-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-lg font-bold text-gray-900", children: t2("settings.title") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-400 mt-1", children: t2("settings.subtitle") })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavButton,
          {
            tab: "general",
            icon: Settings2,
            label: t2("settings.nav.general.label"),
            desc: t2("settings.nav.general.desc")
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavButton,
          {
            tab: "claude",
            icon: Cpu,
            label: t2("settings.nav.claude.label"),
            desc: t2("settings.nav.claude.desc")
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavButton,
          {
            tab: "codex",
            icon: Terminal,
            label: t2("settings.nav.codex.label"),
            desc: t2("settings.nav.codex.desc")
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavButton,
          {
            tab: "antigravity",
            icon: Box,
            label: t2("settings.nav.antigravity.label"),
            desc: t2("settings.nav.antigravity.desc")
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavButton,
          {
            tab: "email",
            icon: Mail,
            label: t2("settings.nav.email.label"),
            desc: t2("settings.nav.email.desc")
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-auto px-4 py-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-2 text-xs", children: isSaving ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "w-3 h-3 animate-spin text-blue-500" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-500", children: t2("settings.status.saving") })
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-400", children: t2("settings.status.synced", { time: lastSaved.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }) })
      ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-400", children: t2("settings.status.upToDate") }) }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 bg-gray-50/50 rounded-2xl border border-gray-100 p-8 overflow-y-auto", children: [
      activeTab === "general" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-1", children: t2("settings.general.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("settings.general.description") })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white p-6 rounded-xl border border-gray-200/60 shadow-sm space-y-6", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-gray-900 block", children: t2("settings.general.launchAtLogin.label") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-500 mt-1", children: t2("settings.general.launchAtLogin.description") })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-gray-900 block", children: t2("settings.general.autoUpdate.label") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-500 mt-1", children: t2("settings.general.autoUpdate.description") })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-gray-900 block", children: t2("settings.language.label") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-500 mt-1", children: t2("settings.language.description") })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "select",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "system", children: t2("settings.language.system") }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "en", children: t2("settings.language.en") }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "zh-TW", children: t2("settings.language.zhTW") })
                ]
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pt-4 border-t border-gray-100", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-4", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-gray-900 block", children: t2("settings.general.softwareUpdate.label") }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-500 mt-1", children: t2("settings.general.softwareUpdate.description") })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    ...
                    t2("settings.general.checkForUpdates")
                  ]
                }
              )
            ] }),
            updateStatus && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
              ...
              !updateStatus.checking && !updateStatus.available && !updateStatus.error && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2 p-3 bg-emerald-50 text-emerald-700 rounded-lg text-xs", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t2("settings.update.upToDate") })
              ] }),
              updateStatus.available && !updateStatus.downloaded && !updateStatus.downloading && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4 bg-blue-50 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-medium text-blue-900", children: t2("settings.update.available", { version: updateStatus.version }) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-blue-700 mt-1", children: updateStatus.updateMethod === "asar" ? t2("settings.update.methodAsar") : t2("settings.update.methodFull") })
                ] }),
                updateStatus.releaseUrl ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "a",
                  {
                    ...
                    children: [
                      ...
                      t2("settings.update.manualDownload")
                    ]
                  }
                ) : /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
              ...
              updateStatus.downloading && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 bg-blue-50 rounded-lg", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 mb-2", children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm font-medium text-blue-900", children: t2("settings.update.downloading") })
                ] }),
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-blue-700 mt-2", children: t2("settings.update.progress", { percent: Math.round(updateStatus.progress) }) })
              ] }),
              updateStatus.downloaded && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4 bg-emerald-50 rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-medium text-emerald-900", children: t2("settings.update.downloaded") }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-emerald-700 mt-1", children: t2("settings.update.restartNote") })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    ...
                    children: [
                      ...
                      t2("settings.update.restartInstall")
                    ]
                  }
                )
              ] }) })
      ...
      activeTab === "claude" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-1", children: t2("settings.claude.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("settings.claude.description") })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white p-6 rounded-xl border border-gray-200/60 shadow-sm space-y-6", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.cli.pathLabel") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "text",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "mt-1.5 text-xs text-gray-400", children: [
              t2("settings.claude.pathNotePrefix"),
              " ",
              /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-gray-100 px-1 rounded", children: "claude" }),
              " ",
              t2("settings.claude.pathNoteSuffix")
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pt-4 border-t border-gray-100 flex items-center gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                className: "px-4 py-2 text-xs font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 disabled:opacity-50 flex items-center gap-2 transition-colors",
                children: [
                  testingClaude ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "w-3.5 h-3.5 animate-spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { className: "w-3.5 h-3.5" }),
                  t2("settings.cli.testConnection")
                ]
              }
            ),
            claudeResult && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-center gap-2 text-xs font-medium ${claudeResult.success ? "text-emerald-600" : "text-red-600"}`, children: [
              claudeResult.success ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { className: "w-4 h-4" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(CircleAlert, { className: "w-4 h-4" }),
              claudeResult.success ? t2("settings.cli.connectionSuccessful") : t2("settings.cli.connectionFailed")
            ] })
          ] }),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                placeholder: t2("settings.codex.placeholder"),
                className: "w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1.5 text-xs text-gray-400", children: t2("settings.cli.defaultPathNote") })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pt-4 border-t border-gray-100 flex items-center gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
            ...
            codexResult && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-center gap-2 text-xs font-medium ${codexResult.success ? "text-emerald-600" : "text-red-600"}`, children: [
              ...
              codexResult.success ? t2("settings.cli.connectionSuccessful") : t2("settings.cli.connectionFailed")
            ] })
          ] }),
      ...
      activeTab === "antigravity" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-1", children: t2("settings.antigravity.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("settings.antigravity.description") })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white p-6 rounded-xl border border-gray-200/60 shadow-sm space-y-6", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.cli.pathLabel") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                ...
                placeholder: t2("settings.antigravity.placeholder"),
                className: "w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1.5 text-xs text-gray-400", children: t2("settings.cli.defaultPathNote") })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pt-4 border-t border-gray-100 flex items-center gap-4", children: [
            ...
            antigravityResult && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `flex items-center gap-2 text-xs font-medium ${antigravityResult.success ? "text-emerald-600" : "text-red-600"}`, children: [
              ...
              antigravityResult.success ? t2("settings.cli.connectionSuccessful") : t2("settings.cli.connectionFailed")
            ] })
          ] }),
      ...
      activeTab === "email" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-2xl space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-semibold text-gray-900 mb-1", children: t2("settings.email.title") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500", children: t2("settings.email.description") })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white p-6 rounded-xl border border-gray-200/60 shadow-sm space-y-6", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.email.smtpHost") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  type: "text",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.email.port") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
                  type: "text",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.email.username") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "text",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.email.password") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "password",
                ...
                placeholder: t2("settings.email.passwordPlaceholder"),
                className: "w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-colors"
              }
            )
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1.5", children: t2("settings.email.fromAddress") }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
                type: "email",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-1.5 text-xs text-gray-400", children: t2("settings.email.fromNote") })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "pt-4 border-t border-gray-100", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-2", children: t2("settings.email.testConfig") }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "input",
                {
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    testingEmail ? /* @__PURE__ */ jsxRuntimeExports.jsx(LoaderCircle, { className: "w-3.5 h-3.5 animate-spin" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Mail, { className: "w-3.5 h-3.5" }),
                    t2("settings.email.sendTest")
                  ]
                }
              )
            ] }),
            emailResult && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `mt-2 text-xs font-medium ${emailResult.success ? "text-emerald-600" : "text-red-600"}`, children: emailResult.message })
          ] })
        ] })
      ] })
    ] })
...
function detectPlatform() {
  ...
  if (ua2.includes("mac")) return "mac";
  if (ua2.includes("win")) return "windows";
  return "unknown";
}
const SLIDE_SOURCES = [
...
const STEP_META = [
  ...
  },
  {
    number: "2",
    key: "writePrompt",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-8 h-8", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" }) }),
  ...
  },
  {
    number: "3",
    key: "setSchedule",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-8 h-8", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
...
const FEATURE_META = [
  {
    key: "smartScheduling",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" }) }),
    color: "text-blue-600",
    bg: "bg-blue-50"
  },
  {
  ...
  {
    key: "executionLogs",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }),
    color: "text-emerald-600",
    bg: "bg-emerald-50"
  },
  {
    key: "emailNotifications",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" }) }),
    color: "text-orange-600",
    bg: "bg-orange-50"
  },
  {
    key: "crossPlatform",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" }) }),
    color: "text-cyan-600",
    bg: "bg-cyan-50"
  },
  {
    key: "autoRetry",
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" }) }),
    color: "text-red-600",
    bg: "bg-red-50"
  }
];
...
function ScreenshotSlider() {
  ...
  const next = reactExports.useCallback(() => setCurrent((i) => (i + 1) % SLIDE_SOURCES.length), []);
  const prev = reactExports.useCallback(() => setCurrent((i) => (i - 1 + SLIDE_SOURCES.length) % SLIDE_SOURCES.length), []);
  reactExports.useEffect(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-16 w-full max-w-5xl mx-auto", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative group", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "rounded-xl overflow-hidden shadow-2xl ring-1 ring-gray-900/10 relative aspect-[16/10] bg-white", children: SLIDE_SOURCES.map((slide, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
        "img",
        {
          src: slide.src,
          alt: t2(slide.altKey),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          "aria-label": t2("welcome.screenshots.prev"),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          "aria-label": t2("welcome.screenshots.next"),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex justify-center gap-2 mt-4", children: SLIDE_SOURCES.map((_, i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        onClick: () => setCurrent(i),
        className: `w-2 h-2 rounded-full transition-all ${i === current ? "bg-blue-600 w-6" : "bg-gray-300 hover:bg-gray-400"}`,
        "aria-label": t2("welcome.screenshots.goTo", { index: i + 1 })
      },
      i
    )) })
  ] });
...
function WelcomePage() {
  ...
  const steps = reactExports.useMemo(
    () => STEP_META.map((s) => ({
      ...s,
      title: t2(`welcome.steps.${s.key}.title`),
      description: t2(`welcome.steps.${s.key}.desc`)
    })),
    [t2]
  );
  const features = reactExports.useMemo(
    () => FEATURE_META.map((f2) => ({
      ...f2,
      title: t2(`welcome.features.${f2.key}.title`),
      description: t2(`welcome.features.${f2.key}.desc`)
    })),
    [t2]
  );
  const comparison = reactExports.useMemo(
    () => COMPARISON_KEYS.map((key) => ({
      label: t2(`welcome.comparison.${key}.label`),
      others: t2(`welcome.comparison.${key}.others`),
      orbit: t2(`welcome.comparison.${key}.orbit`)
    })),
    [t2]
  );
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen bg-[#F8F7F6] flex flex-col relative overflow-hidden", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-blue-200/20 rounded-full blur-[120px]" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] bg-cyan-200/20 rounded-full blur-[120px]" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("header", { className: "sticky top-0 z-50 bg-white/70 backdrop-blur-lg border-b border-gray-200/50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto px-6 h-16 flex items-center justify-between", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-9 h-9 flex items-center justify-center bg-white rounded-xl shadow-sm ring-1 ring-black/5", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { className: "w-7 h-7 text-blue-600", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "3", className: "fill-blue-600 stroke-none" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", strokeLinecap: "round", className: "opacity-80", d: "M12 21c-4.97 0-9-4.03-9-9s4.03-9 9-9c4.97 0 9 4.03 9 9" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", className: "text-cyan-400", d: "M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12", transform: "rotate(-45 12 12)" })
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-lg font-bold text-gray-900 font-display", children: "Orbit Agents" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "a",
          {
            ...
            "aria-label": "GitHub",
            children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-5 h-5", fill: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { fillRule: "evenodd", d: "M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z", clipRule: "evenodd" }) })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "a",
          {
            ...
            children: t2("welcome.nav.download")
          }
        )
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 pt-20 pb-16 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col items-center gap-4 mb-8", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-blue-700 text-sm font-medium", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "w-2 h-2 rounded-full bg-blue-500 motion-safe:animate-pulse" }),
          t2("welcome.hero.badge")
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "a",
          {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("h1", { className: "text-5xl md:text-7xl font-bold font-display text-gray-900 tracking-tight mb-6 leading-[1.1]", children: [
        t2("welcome.hero.title"),
        " ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-cyan-600", children: t2("welcome.hero.titleHighlight") })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xl md:text-2xl text-gray-600 mb-12 max-w-2xl mx-auto leading-relaxed font-display", children: t2("welcome.hero.subtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row items-center sm:items-start justify-center gap-4", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "a",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", fill: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { fillRule: "evenodd", d: "M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z", clipRule: "evenodd" }) }),
              t2("welcome.hero.viewGitHub")
            ]
          }
        )
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-center gap-3 mt-4", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("a", { href: WIN_DOWNLOAD_URL, className: "text-sm text-gray-500 hover:text-blue-600 font-medium transition-colors flex items-center gap-1", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-4 h-4", viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M0 3.449L9.75 2.1v9.451H0m9.75 9.413L0 19.488V12h9.75m2.25-10.413L24 0v11.458H12M24 24l-12-1.583v-9.967H24" }) }),
          t2("welcome.hero.downloadWindows")
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-300", children: "|" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: WIN_ZIP_DOWNLOAD_URL, className: "text-sm text-gray-500 hover:text-blue-600 font-medium transition-colors", children: t2("welcome.hero.downloadPortable") })
      ] }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 py-24", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-5xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-3xl md:text-4xl font-bold text-gray-900 text-center mb-4 font-display", children: t2("welcome.steps.sectionTitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg text-gray-500 text-center mb-16 max-w-xl mx-auto", children: t2("welcome.steps.sectionSubtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid md:grid-cols-3 gap-8", children: steps.map((step) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `w-12 h-12 rounded-xl bg-gradient-to-br ${step.color} text-white flex items-center justify-center text-lg font-bold mb-6 shadow-lg`, children: step.number }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-xl font-bold text-gray-900 mb-3 font-display", children: step.title }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 py-24 bg-white/50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-3xl md:text-4xl font-bold text-gray-900 text-center mb-4 font-display", children: t2("welcome.comparison.sectionTitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg text-gray-500 text-center mb-16 max-w-xl mx-auto", children: t2("welcome.comparison.sectionSubtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto rounded-2xl ring-1 ring-gray-900/5 shadow-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full min-w-[500px] text-left", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "bg-gray-50", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", "aria-label": t2("welcome.comparison.featureColLabel"), className: "px-6 py-4 text-sm font-semibold text-gray-500 uppercase tracking-wider w-1/4" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", className: "px-6 py-4 text-sm font-semibold text-gray-500 uppercase tracking-wider", children: t2("welcome.comparison.othersHeader") }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", className: "px-6 py-4 text-sm font-semibold text-blue-600 uppercase tracking-wider bg-blue-50/50", children: "Orbit Agents" })
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: comparison.map((row, i) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: i % 2 === 0 ? "bg-white" : "bg-gray-50/50", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-6 py-4 text-sm font-semibold text-gray-700", children: row.label }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-6 py-4 text-sm text-gray-500", children: row.others }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-6 py-4 text-sm font-medium text-gray-900 bg-blue-50/30", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-4 h-4 text-emerald-500 flex-shrink-0", fill: "currentColor", viewBox: "0 0 20 20", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { fillRule: "evenodd", d: "M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z", clipRule: "evenodd" }) }),
            row.orbit
          ] }) })
        ] }, row.label)) })
      ] }) })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 py-24", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-5xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-3xl md:text-4xl font-bold text-gray-900 text-center mb-4 font-display", children: t2("welcome.features.sectionTitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg text-gray-500 text-center mb-16 max-w-xl mx-auto", children: t2("welcome.features.sectionSubtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid md:grid-cols-2 lg:grid-cols-3 gap-6", children: features.map((feature) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          className: "bg-white rounded-2xl p-6 shadow-sm ring-1 ring-gray-900/5 hover:shadow-md transition-shadow duration-300",
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `w-11 h-11 ${feature.bg} rounded-xl flex items-center justify-center ${feature.color} mb-4`, children: feature.icon }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-lg font-bold text-gray-900 mb-2 font-display", children: feature.title }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-600 leading-relaxed", children: feature.description })
          ]
        },
        feature.key
      )) })
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 py-24 bg-white/50", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-4xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-3xl md:text-4xl font-bold text-gray-900 text-center mb-4 font-display", children: t2("welcome.installation.sectionTitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg text-gray-500 text-center mb-12 max-w-xl mx-auto", children: t2("welcome.installation.sectionSubtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid md:grid-cols-2 gap-6", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white rounded-2xl p-8 ring-1 ring-gray-900/5 shadow-sm", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 mb-6", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-xl font-bold text-gray-900 font-display", children: t2("welcome.installation.macos.title") })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-600 mb-4", children: t2("welcome.installation.macos.desc") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-semibold text-gray-800 mb-2", children: t2("welcome.installation.macos.option1Title") }),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm font-semibold text-gray-800 mb-2", children: t2("welcome.installation.macos.option2Title") }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("ol", { className: "list-decimal list-inside space-y-1 text-sm text-gray-600", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("welcome.installation.macos.option2Step1") }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("welcome.installation.macos.option2Step2") }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("welcome.installation.macos.option2Step3") })
              ] })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-400 pt-1", children: t2("welcome.installation.macos.note") })
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white rounded-2xl p-8 ring-1 ring-gray-900/5 shadow-sm", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3 mb-6", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { className: "text-xl font-bold text-gray-900 font-display", children: t2("welcome.installation.windows.title") })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-600 mb-4", children: t2("welcome.installation.windows.desc") }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("ol", { className: "list-decimal list-inside space-y-1 text-sm text-gray-600", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("welcome.installation.windows.step1") }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: t2("welcome.installation.windows.step2") })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs text-gray-400 pt-4", children: t2("welcome.installation.windows.note") })
          ] })
        ] })
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("section", { className: "relative z-10 py-20 text-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-3xl mx-auto px-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { className: "text-3xl md:text-4xl font-bold text-gray-900 mb-4 font-display", children: t2("welcome.cta.title") }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-lg text-gray-500 mb-10", children: t2("welcome.cta.subtitle") }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row items-center justify-center gap-4", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "a",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-6 h-6", viewBox: "0 0 24 24", fill: "currentColor", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M0 3.449L9.75 2.1v9.451H0m9.75 9.413L0 19.488V12h9.75m2.25-10.413L24 0v11.458H12M24 24l-12-1.583v-9.967H24" }) }),
              t2("welcome.cta.downloadWindows")
            ]
          }
        )
      ] })
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("footer", { className: "relative z-10 py-8 border-t border-gray-200/40 bg-white/30 backdrop-blur-sm mt-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-sm text-gray-500", children: [
        t2("welcome.footer.creditPre"),
        " ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("a", { href: "https://www.facebook.com/mukispace", target: "_blank", rel: "noopener noreferrer", className: "hover:text-gray-900 transition-colors", children: "Muki Wu" }),
        t2("welcome.footer.creditPost")
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-6", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "a",
          {
            ...
            "aria-label": "Discord",
            children: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-5 h-5", fill: "currentColor", viewBox: "0 0 24 24", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" }) })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "a",
          {
            ...
            children: t2("welcome.footer.download")
          }
        )
      ] })
    ] }) })
...
function App() {
  const [isElectron] = reactExports.useState(() => window.electronApi !== void 0);
  ...
  const [currentView, setCurrentView] = reactExports.useState("tasks");
  const [editingTask, setEditingTask] = reactExports.useState(null);
  const [showTaskForm, setShowTaskForm] = reactExports.useState(false);
  const [taskListKey, setTaskListKey] = reactExports.useState(0);
  if (!isElectron) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(WelcomePage, {});
  }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-screen bg-[#F8F7F6]", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-56 bg-[#F8F7F6] flex flex-col border-r border-gray-200/60", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-12 drag-region" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-6 pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "relative flex items-center justify-center w-8 h-8", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { className: "w-8 h-8 text-blue-600", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("circle", { cx: "12", cy: "12", r: "3", className: "fill-blue-600 stroke-none" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", strokeLinecap: "round", className: "opacity-80", d: "M12 21c-4.97 0-9-4.03-9-9s4.03-9 9-9c4.97 0 9 4.03 9 9" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", strokeLinecap: "round", className: "opacity-60", d: "M19.07 4.93L4.93 19.07" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", className: "text-cyan-400", d: "M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12", transform: "rotate(-45 12 12)" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeWidth: "1.5", className: "text-blue-500", d: "M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12", transform: "rotate(45 12 12)" })
        ] }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-lg font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-cyan-600", children: "Orbit Agents" })
      ] }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-3 pt-2 pb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "flex-1 px-3 space-y-0.5", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavItem,
          {
            label: t2("app.nav.tasks"),
            icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-[18px] h-[18px]", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.75, d: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" }) }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavItem,
          {
            label: t2("app.nav.executionLogs"),
            icon: /* @__PURE__ */ jsxRuntimeExports.jsx("svg", { className: "w-[18px] h-[18px]", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.75, d: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" }) }),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          NavItem,
          {
            label: t2("app.nav.settings"),
            icon: /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", { className: "w-[18px] h-[18px]", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.75, d: "M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.75, d: "M15 12a3 3 0 11-6 0 3 3 0 016 0z" })
            ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-4 text-sm text-gray-400", children: "v1.0.16" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 flex flex-col overflow-hidden min-w-0", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 overflow-hidden", children: [
        currentView === "tasks" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-full bg-white rounded-tl-2xl shadow-sm border-t border-l border-gray-200/60 p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          TaskList,
          {
        ...
        currentView === "logs" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-full bg-white rounded-tl-2xl shadow-sm border-t border-l border-gray-200/60 p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ExecutionLog, {}) }),
        currentView === "settings" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-full bg-white rounded-tl-2xl shadow-sm border-t border-l border-gray-200/60 p-6", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Settings, {}) })
      ] })
    ] })
  ] });
}
function NavItem({ label, icon, active, onClick }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    ...
    {
      onClick,
      className: `w-full flex items-center gap-2.5 px-3 py-1.5 rounded-lg transition-all text-sm ${active ? "bg-white shadow-sm text-gray-900 font-medium border border-gray-200/60" : "text-gray-600 hover:bg-white/60"}`,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: active ? "text-gray-700" : "text-gray-500", children: icon }),
        label
      ]
    }
  );
